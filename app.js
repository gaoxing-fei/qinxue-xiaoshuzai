...
const UI = {
  ...
  showInstallHelp() {
    ...
    if (AppState.isIOS && !AppState.isSafari) {
      ...
      msg = '🤖 华为 / 安卓手机安装说明：\n\n• Edge 浏览器只能“添加到主屏幕”的快捷方式（带浏览器栏），不是独立 APP。\n• 请用「华为浏览器」或「Chrome」打开本链接：\n  - 华为浏览器：点「⋮」→ 添加到主屏幕（全屏独立窗口）\n  - Chrome：点「⋮」→ 安装应用（真 APP 图标）\n• 想要真正的安装包(APK)：在首页点“打开 PWABuilder 生成安卓安装包”按钮，下载后安装即可。';
    }
    alert(msg);
  },
